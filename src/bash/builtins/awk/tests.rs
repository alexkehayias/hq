...
#[rustfmt::skip]
use crate::bash::builtins::limits::{
...
use crate::bash::builtins::{Builtin, Context};
...
async fn no_leak_unbalanced_brace() {
    let r = crate::bash::builtins::debug_leak_check::run("echo 1 | awk 'BEGIN { print'").await;
    crate::bash::builtins::debug_leak_check::assert_no_leak(&r, "awk_unbalanced_brace", AWK_BANNED);
}

#[tokio::test]
async fn no_leak_invalid_regex() {
    let r = crate::bash::builtins::debug_leak_check::run("echo 1 | awk '/[/'").await;
    crate::bash::builtins::debug_leak_check::assert_no_leak(&r, "awk_invalid_regex", AWK_BANNED);
}
