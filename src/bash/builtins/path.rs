...
fn readlink_path_within_limits(fs: &dyn crate::bash::fs::FileSystem, path: &Path) -> bool {
...
async fn follow_readlink_symlinks(
    ...
    loop {
        ...
        let normalized = crate::bash::fs::normalize_path(&path);
        ...
        for (idx, component) in components.iter().enumerate() {
            ...
            if let Ok(target) = fs.read_link(&current).await {
                ...
                path = crate::bash::fs::normalize_path(&next);
                followed = true;
                break;
            }
        }
...
mod tests {
    ...
    async fn test_readlink_respects_stricter_filesystem_path_limit() {
        ...
        let strict_fs = Arc::new(InMemoryFs::with_limits(
            crate::bash::fs::FsLimits::default().max_path_length(50),
