...
mod tests {
    ...
    async fn run(args: &[&str], stdin: Option<&str>, fs: Option<Arc<InMemoryFs>>) -> ExecResult {
        ...
        let fs_dyn = fs as Arc<dyn crate::bash::fs::FileSystem>;
        let ctx = Context {
            args: &args,
            env: &env,
    ...
    async fn test_read_from_file() {
        ...
        let fs_dyn = fs.clone() as Arc<dyn crate::bash::fs::FileSystem>;
        fs_dyn
    ...
    async fn no_leak_malformed_input() {
        let r = crate::bash::builtins::debug_leak_check::run(r#"echo 'not json' | json get .foo"#).await;
