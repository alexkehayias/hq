...
async fn try_indexed_search(
    fs: &dyn crate::bash::fs::FileSystem,
    opts: &GrepOptions,
    cwd: &std::path::Path,
    ...
    for file in &opts.files {
        let root = crate::bash::fs::normalize_path(&if file.starts_with('/') {
            std::path::PathBuf::from(file)
        } else {
            cwd.join(file)
        });
        ...
        let query = crate::bash::fs::SearchQuery {
            pattern,
        ...
        for m in &results.matches {
            let candidate = if m.path.is_absolute() {
                crate::bash::fs::normalize_path(&m.path)
            } else {
                crate::bash::fs::normalize_path(&root.join(&m.path))
            };
...
mod tests {
    ...
    impl FileSystem for IndexedTestFs {
        ...
        async fn stat(&self, path: &Path) -> Result<crate::bash::fs::Metadata> {
            self.inner.stat(path).await
        }
        async fn read_dir(&self, path: &Path) -> Result<Vec<crate::bash::fs::DirEntry>> {
    ...
    async fn no_leak_invalid_regex() {
        let r = crate::bash::builtins::debug_leak_check::run(r"echo 1 | grep -E '['").await;
