...
use crate::bash::interpreter::{ExecResult, is_internal_variable};
...
mod tests {
    ...
    async fn no_leak_printf_format_errors() {
        let r = crate::bash::builtins::debug_leak_check::run("printf '%10001s' x").await;
        crate::bash::builtins::debug_leak_check::assert_no_leak(&r, "printf_width_cap", &[]);
    }
    ...
    fn no_leak_all_format_error_variants() {
        ...
        for err in variants {
            ...
            crate::bash::testing::assert_no_leak(&result, "printf_format_error_variant", &[]);
        }
    }
}
