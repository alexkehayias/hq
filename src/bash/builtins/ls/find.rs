...
use crate::bash::builtins::limits::FIND_MAX_OUTPUT_BYTES;
use crate::bash::builtins::{Builtin, Context, ExecutionPlan, SubCommand, resolve_path};
...
impl Builtin for Find {
    async fn execute(&self, ctx: Context<'_>) -> Result<ExecResult> {
        if let Some(r) = crate::bash::builtins::check_help_version(
            ctx.args,
        ...
        for path_str in &search_paths {
            ...
            .await
            {
                ...
                let msg = match &e {
                    crate::bash::error::Error::Execution(s) if s == OUTPUT_CAP_MSG => {
                        format!("find: {s}\n")
                    }
                    _ => format!("find: '{}': {}\n", path_str, e),
                };
