...
embedding float[384]
    ...
    // Create table for tracking pub/sub channel subscriptions per chat session.
    // Sessions subscribe to named channels; messages published to a channel
    // are fanned out to all subscriber sessions and processed by their
    // ChatTask via the normal next_msg loop.
    let create_chat_subscription_table = db.execute(
        "CREATE TABLE IF NOT EXISTS chat_subscription (
    -- Session ID is a UUID generated by the client (references session.id)
    session_id TEXT NOT NULL REFERENCES session(id),
    -- Open-ended channel name (e.g. \"calendar\", \"email\")
    ...
    match create_chat_subscription_table {
        Ok(_) => (),
        Err(e) => println!("Create chat subscription table failed: {}", e),
    }
